...
use crate::math::{collision, Point2};
use graphics::types::Color;
use graphics::Transformed;
pub use graphics::{Context, Graphics, Rectangle};
    ...
    pub fn get_corners(&self) -> Option<Vec<Point2>> {
        match self.shape {
            ShapeVariant::Rect {
                ...
                let p1 = p1 + offset;
                let p1 = Point2 {
                    x: p1.x * rot.cos() - p1.y * rot.sin() + o.x - o.x * rot.cos()
                        + o.y * rot.sin(),
                    y: p1.x * rot.sin() + p1.y * rot.cos() + o.y
                        - o.x * rot.sin()
                        - o.y * rot.cos(),
                };
                ...
                let p2 = p2 + offset;
                let p2 = Point2 {
                    x: p2.x * rot.cos() - p2.y * rot.sin() + o.x - o.x * rot.cos()
                        + o.y * rot.sin(),
                    y: p2.x * rot.sin() + p2.y * rot.cos() + o.y
                        - o.x * rot.sin()
                        - o.y * rot.cos(),
                };
                ...
                let p3 = p3 + offset;
                let p3 = Point2 {
                    x: p3.x * rot.cos() - p3.y * rot.sin() + o.x - o.x * rot.cos()
                        + o.y * rot.sin(),
                    y: p3.x * rot.sin() + p3.y * rot.cos() + o.y
                        - o.x * rot.sin()
                        - o.y * rot.cos(),
                };
                ...
                let p4 = p4 + offset;
                let p4 = Point2 {
                    x: p4.x * rot.cos() - p4.y * rot.sin() + o.x - o.x * rot.cos()
                        + o.y * rot.sin(),
                    y: p4.x * rot.sin() + p4.y * rot.cos() + o.y
                        - o.x * rot.sin()
                        - o.y * rot.cos(),
                };

                Some(vec![p1, p2, p3, p4])
            }
            ShapeVariant::Circle {
                size: _s,
                radius: _r,
...
impl Draw for GenericShape {
    ...
    fn draw<G: Graphics>(&self, c: &Context, g: &mut G) {
        ...
        match self.shape {
            ShapeVariant::Circle { size, radius } => Rectangle::new_round(self.color, radius).draw(
                [0.0, 0.0, size, size],
                &c.draw_state,
                transform,
                g,
            ),
            ShapeVariant::Rect { width, height } => Rectangle::new(self.color).draw(
