...
use crate::game::consts::{
    map_idx_to_point2, point2_to_map_idx, PLAYER_SIZE, TILE_SIZE, TOWER_ATTACK_COOLDOWN,
};
...
impl GameController {
            ...
            if self.keys_pressed.contains(&key) {
                ...
                match key {
                    Key::Space => {
                        self.model
                            .player
                            .change_state(player::PlayerState::FinishedAttacking);
                    }
                    _ => (),
                }
            }
    ...
    pub fn tick(&mut self, dt: f64) {
        ...
        if self.keys_pressed.contains(&Key::W) {
            ...
            self.model
                .player
                .change_state(player::PlayerState::Stationary);
        }
        // Start animation if Space is pressed
        if self.keys_pressed.contains(&Key::Space) {
            self.model
                .player
                .change_state(player::PlayerState::Attacking);
        }
    ...
    fn check_player_collision(&mut self) {
        ...
        let max_idx = point2_to_map_idx(
            self.model.player.shape.get_position()
                + Point2 {
                    x: PLAYER_SIZE,
                    y: PLAYER_SIZE,
                },
                ...
                if let Some(tile) = self.model.level.map.get(&MapIdx::new(w, h)) {
                    match tile.variant {
                        TileVariant::Wall => {
                            ...
                            let shift_left = tile_pos.x
                                - self.model.player.shape.get_position().x
                                - PLAYER_SIZE
                                - 0.1;
                            let shift_right = tile_pos.x + TILE_SIZE
                                - self.model.player.shape.get_position().x
                                + 0.1;
                            let shift_up = tile_pos.y
                                - self.model.player.shape.get_position().y
                                - PLAYER_SIZE
                                - 0.1;
                            let shift_down = tile_pos.y + TILE_SIZE
                                - self.model.player.shape.get_position().y
                                + 0.1;
    ...
    fn tick_enemies(&mut self, dt: f64) {
        ...
        if self.model.beacon.health == 0 || self.model.player.health == 0 {
            println!("Game Over!");
            println!(
                "Player Health: {}, Beacon Health: {}",
                self.model.player.health, self.model.beacon.health
            );
            ...
            match tower.state {
                TowerState::Attacking => {
                    if let Some(tile) = self
                        ...
                        .level
                        .map
                        .get(&point2_to_map_idx(tower.bullet.shape.center_point()))
                    {
                        match tile.variant {
                            TileVariant::Wall => {
